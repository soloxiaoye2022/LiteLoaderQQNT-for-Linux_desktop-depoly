...
check_nodejs() {
    nodejs_version=$(node -v| cut -d "." -f 1 | sed 's/[^0-9]//g')
    npm_version=$(npm -v| cut -d "." -f 1 | sed 's/[^0-9]//g')
    [[ -z "${nodejs_version}" ]] && echo -e "${Error} 当前系统未安装 nodejs ，请安装nodejs 16+，即将退出脚本。" && sleep 5 && exit 1
    [[ ! ${nodejs_version} == "16" ]] && echo -e "${Error} 当前系统安装的nodejs ${nodejs_version} 版本过低，请安装nodejs 16+，即将退出脚本。" && sleep 5 && exit 1
...
LinuxQQ_install() {
    ...
    sudo apt install wget curl gnupg git nohup -y
	...
	if [ $? = 0 ] ; then
        ...
        sudo chown -R ${user}:${groups} /opt/QQ/
	else
...
LiteLoader_install() {	
    ...
    cd /tmp/
    wget ${ghproxy}https://raw.githubusercontent.com/soloxiaoye2022/LiteLoaderQQNT-for-Linux-depoly/main/LiteLoaderQQNT-Plugin-Chronocat.tar.gz
    ...
    sudo chown -R ${user}:${groups} /home/${user}/Documents/LiteLoaderQQNT/
    echo -e "${Info} LinuxQQ 安装完成！即将启动QQ，请扫码登录Bot账号。如QQ未弹窗请手动启动QQ。" 
    sleep 3
    sudo killall -HUP qq && sudo -u ${user} nohup qq& > /dev/null 2>&1 & #启动LinuxQQ
...
EOF
    echo -e "${Info} 配置成功..."
    sudo chown -R ${user}:${groups} /opt/Yunzai/
